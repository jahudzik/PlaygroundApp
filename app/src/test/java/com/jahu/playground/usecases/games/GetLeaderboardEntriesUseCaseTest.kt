...
class GetLeaderboardEntriesUseCaseTest {
    ...
    fun execute_empty() {
        ...
        assertEquals(emptyList<LeaderboardEntry>(), resultList)
    }

    @Test
    fun execute_singleUser() {
    ...
    fun execute_multipleUsers() {
        val userGames = buildGameResultsList(testUsers[0].nick, 7, 6, 1, 4, 5)
                ...
                .plus(buildGameResultsList(testUsers[2].nick, 10, 9, 8, 9))
        ...
        val expectedResult = listOf(
                LeaderboardEntry(testUsers[2].nick, 9.0, 4),
    ...
    private fun buildGameResultsList(userNick: String, vararg results: Int): List<GameResult> {
        return results.map { GameResult(userNick, it, 100) }
    }
